...
task cellprofiler_pipeline_task {
  ...
  input {
    ...
    # File-related inputs
    Array[File] images_files
    Array[File] illum_files
    File? load_data_csv
  ...
  command {
    ...
    cromwell_image_dir=$(dirname ~{images_files[0]})
    ...
    # locate the directory with the illumination images
    cromwell_illum_dir=$(dirname ~{illum_files[0]})

    # move the illum images to a precisely-known path
    mkdir ~{illum_image_dir}
    mv $cromwell_illum_dir/*.npy ~{illum_image_dir}

    echo "Directory where we moved the illumination files =================="
    echo ~{illum_image_dir}
    ls -lah ~{illum_image_dir}

    echo "Directory where we moved the images files =================="
